			...
			if (!caCheckEmailAddress($ps_email)) {
				...
				if ($t_user->getPrimaryKey()) {
					...
					if (sizeof($va_errors) == 0) {	
						...
						# --- get the subject fo the email from template
						ob_start();
						require($this->request->getViewsDirectoryPath()."/mailTemplates/instructions_subject.tpl");
						$vs_subject_line = ob_get_contents(); 
						ob_end_clean();
						
						$vs_action = "reset";
						$vs_password_reset_url = $this->request->config->get("site_host").caNavUrl($this->request, '', 'LoginReg', 'resetSave', array('key' => $vs_reset_key));
						# -- generate mail text from template - get botht eh text and html versions
						ob_start();
						require($this->request->getViewsDirectoryPath()."/mailTemplates/instructions.tpl");
						$vs_mail_message_text = ob_get_contents(); 
						ob_end_clean();
						ob_start();
						require($this->request->getViewsDirectoryPath()."/mailTemplates/instructions_html.tpl");
						$vs_mail_message_html = ob_get_contents(); 
						ob_end_clean();
					...
					case 'reset_save':
						...
						if ($t_user->getPrimaryKey()) {
							...
							if ($t_user->numErrors()) {
								...
								ob_start();
								require($this->request->getViewsDirectoryPath()."/mailTemplates/notification_subject.tpl");
								$vs_subject_line = ob_get_contents(); 
								ob_end_clean();
								...
								ob_end_clean();
								ob_start();
								require($this->request->getViewsDirectoryPath()."/mailTemplates/notification_html.tpl");
								$vs_mail_message_html = ob_get_contents(); 
								ob_end_clean();
								caSendmail($t_user->get('email'), $this->request->config->get("ca_admin_email"), $vs_subject_line, $vs_mail_message_text, $vs_mail_message_html);
							}
