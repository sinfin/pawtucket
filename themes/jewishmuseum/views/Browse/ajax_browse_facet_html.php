 ...
 */
	...
	// print "<pre>"; var_dump($this->getAllVars()); print "</pre>";
...
<!-- <div id="title"><?php print unicode_ucfirst($va_facet_info['label_singular']); ?></div> -->
...
<?php	
	...
	// $va_facet_info['group_mode'] = 'alphabetical';
...
<?php
			foreach($va_facet as $vn_i => $va_item) {
				$vs_label = caGetLabelForDisplay($va_facet, $va_item, $va_facet_info);
				print "<li class=\"year-item\">".caNavLink($this->request, $vs_label, 'browseSelectPanelLink', $this->request->getModulePath(), $this->request->getController(), ((strlen($vm_modify_id)) ? 'modifyCriteria' : 'addCriteria'), array('facet' => $vs_facet_name, 'id' => $va_item['id'], 'mod_id' => $vm_modify_id))."</li>";
			}
?>
		</ul>
	</div>
...
<?php
			...
			foreach($va_grouped_items as $vs_group => $va_items) {
				...
				print "<div class=\"letter-wrap\"><div class='browseSelectPanelListGroupHeading'><a name='{$vs_group}' class='browseSelectPanelListGroupHeading'>{$vs_group}</a></div>\n";
?>
...
<?php
				...
				foreach($va_items as $va_item) {
					$vs_label = caGetLabelForDisplay($va_facet, $va_item, $va_facet_info);
					$link = caNavLink($this->request, $vs_label, 'browseSelectPanelLink', $this->request->getModulePath(), $this->request->getController(), ((strlen($vm_modify_id) > 0) ? 'modifyCriteria' : 'addCriteria'), array('facet' => $vs_facet_name, 'id' => $va_item['id'], 'mod_id' => $vm_modify_id));
